...
def plot_avg_progress(rows: list[Row], output_dir: Path) -> Path:
    ...
    for bar, value in zip(bars, values):
        ax.text(bar.get_x() + bar.get_width() / 2, value + 0.006, f"{value:.4f}", ha="center", va="bottom", fontsize=9)
...
def plot_prompt_ablation(rows: list[Row], output_dir: Path) -> Path:
    ...
    ax.legend(frameon=False, loc="upper center", bbox_to_anchor=(0.5, 1.04), ncol=2, borderaxespad=0.0)
    _style_axis(ax)
