...
class DatasetDefinition:
    """Metadata required to construct a task-stream dataset adapter."""
...
DATASET_DEFINITIONS: dict[str, DatasetDefinition] = {
    "alfworld": DatasetDefinition(
        dataset_name="alfworld",
        relative_path=Path("data/aomp_mem/processed/alfworld.jsonl"),
        loader=AlfWorldDataset,
    ),
}


def code_root() -> Path:
    """Return the migrated ALFWorld root for data and results path resolution."""
...
def load_dataset(
    ...
    *,
    ...
    prompt_mode: str = AlfWorldDataset.PROMPT_MODE_BENCHMARK,
) -> BaseTaskStreamDataset:
    ...
    if dataset_name == "alfworld":
        return definition.loader(dataset_path, prompt_mode=prompt_mode)
...
def available_method_names() -> list[str]:
    """Return the schedulers used by the ALFWorld main experiments."""
