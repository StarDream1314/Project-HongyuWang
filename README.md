...
本仓库整理了四组围绕 A-OMP / A-OMP-Mem、双通道反馈和慢反馈调度的实验。整体结构按实验对象拆分，每个子目录都保留了可复现实验脚本、输入数据或中间产物、运行结果、图表和单独的 `README.md`。

根目录 README 用于快速导航。更完整的参数说明、实验背景和结果解释请优先查看各子项目 README 与 `Record.md`。
