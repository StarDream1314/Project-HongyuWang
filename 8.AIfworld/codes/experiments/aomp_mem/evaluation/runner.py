...
def _run_alfworld_track(
    *,
    ...
    dataset = load_dataset("alfworld", root=_code_root())
    ...
    for seed in seeds:
        for method_name in method_names:
            ...
            result_group = _alfworld_result_group(method_name, scienceworld_prompt_mode)
            run_dir = _alfworld_run_dir(
                seed=int(seed),
                scheduler=method_name,
                prompt_mode=scienceworld_prompt_mode,
            ...
            runner = ExperimentRunner(
                config=ExperimentConfig(
                    ...
                    extra={
                        ...
                        "phase": "phase2",
                        "backend": backend,
                        "alfworld_prompt_mode": scienceworld_prompt_mode,
...
def main() -> None:
    ...
    parser.add_argument("--track", required=True, choices=["exact_match", "alfworld", "scienceworld_family_prior"])
    ...
    parser.add_argument(
        ...
        help="Compatibility no-op retained for old ScienceWorld commands.",
    )
    ...
    track = "alfworld" if args.track == "scienceworld_family_prior" else args.track
